...
- (void)loadUnderlyingImageAndNotify {
    ...
    if (self.underlyingImage) {
        ...
        [[SDImageCache sharedImageCache] storeImage:self.underlyingImage forKey:[_photoURL.absoluteString MD5]];
        ...
        if (_photoPath) {
            ...
            [[SDWebImageManager sharedManager] downloadImageWithURL:_photoURL options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                ...
                if (image) {
                    self.underlyingImage = image;
                    [[SDImageCache sharedImageCache] storeImage:image forKey:[_photoURL.absoluteString MD5]];
                    POST_NOTIFICATION(kRefreshView, @{@"updatePhoto":imageURL});
                }
                [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
            }];
            
            /*
            [[SDWebImageManager sharedManager] downloadWithURL:_photoURL options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
				CGFloat progress = ((CGFloat)receivedSize)/((CGFloat)expectedSize);

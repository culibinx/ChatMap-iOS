...
{
    NSString *title = TRANSLATE(@"Undo Typing");
    NSString *acceptTitle = TRANSLATE(@"Undo");
    NSString *cancelTitle = TRANSLATE(@"Cancel");
...
- (void)slk_registerKeyCommands
{
    ...
    // Enter Key
    [self.textView observeKeyInput:@"\r" modifiers:0 title:TRANSLATE(@"Send/Accept") completion:^(UIKeyCommand *keyCommand) {
        [weakSelf didPressReturnKey:keyCommand];
    }];
    
    // Esc Key
    [self.textView observeKeyInput:UIKeyInputEscape modifiers:0 title:TRANSLATE(@"Dismiss") completion:^(UIKeyCommand *keyCommand) {

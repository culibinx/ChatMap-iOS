...
{
    UIMenuItem *undo = [[UIMenuItem alloc] initWithTitle:TRANSLATE(@"Undo") action:@selector(slk_undo:)];
    UIMenuItem *redo = [[UIMenuItem alloc] initWithTitle:TRANSLATE(@"Redo") action:@selector(slk_redo:)];
    
    NSMutableArray *items = [NSMutableArray arrayWithObjects:undo, redo, nil];
    
    if (self.registeredFormattingTitles.count > 0) {
        UIMenuItem *format = [[UIMenuItem alloc] initWithTitle:TRANSLATE(@"Format") action:@selector(slk_presentFormattingMenu:)];
